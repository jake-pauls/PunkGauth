...
echo -e "${YELLOW}/server: Checking packages on the server..."
cd server
yarn

echo -e "${YELLOW}/server: Rebuilding TypeScript..."
yarn build

echo "${GREEN}/client: Checking packages on the client..."
cd .. && cd client
yarn

echo "${BLUE}docker: Cleaning dangling Docker images..."
cd .. && docker rmi $(docker images -aq -f 'dangling=true')

echo "${BLUE}docker: Relaunching Docker compose..."
docker-compose up
